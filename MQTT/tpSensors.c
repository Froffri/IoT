...
static void sensReadRestart(void* ptr) {
  process_start(&sensRead, NULL);
}
...
static void msg_handler(const char *topic, uint16_t topic_len, const uint8_t *chunk, uint16_t chunk_len) {
  ...
  LOG_INFO("Message received: topic='%s' (len=%u), chunk_len=%u\n", topic, topic_len, chunk_len);
    ...
    ctimer_set(&restart_timer, CLOCK_SECOND * 20, &sensReadRestart, NULL);
    exit(0);
...
static void changeCp(){
  ...
  int randomIncrement = (rand() % (4) - 1); // {-1, 0, 1, 2}
  ...
  if(overload)
    cp -= (rand() % (3) + 1) * 1000; // {1000, 2000, 3000}
...
static void readnSend(){
  ...
  mqtt_publish(&conn, NULL, pub_topic[0], (uint8_t *)app_buffer,
  strlen(app_buffer), MQTT_QOS_LEVEL_0, MQTT_RETAIN_OFF);

  memset(app_buffer, 0, APP_BUFFER_SIZE);
  sprintf(app_buffer, "{\"subzone\": %d, \"consumed_power\": %d}", SUBZONE, cpSend);

  mqtt_publish(&conn, NULL, pub_topic[1], (uint8_t *)app_buffer,
  strlen(app_buffer), MQTT_QOS_LEVEL_0, MQTT_RETAIN_OFF);
}
/*---------------------------------------------------------------------------*/
...
PROCESS_THREAD(sensRead, ev, data) {
  ...
  cp = (rand() % (UP_POW - DOWN_POW + 1)) + DOWN_POW;
